    ...
    function renderScreen1() {
      ...
      if (mode === "Artist") {
        ...
        let listContent = '';
        if (hasResults) {
          listContent = artistList.map(a => `<div class="autocomplete-item" onmousedown="acSelect('searchTermSel','acListArtist','${esc(a).replace(/'/g, "\\'")}',dispatchSearchAndRender)"><span class="ac-icon">${esc(a)[0]}</span>${esc(a)}</div>`).join('');
        } else if (hasInput) {
